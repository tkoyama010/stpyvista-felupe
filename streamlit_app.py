...
if material_type == "Custom":
    ...
    nu = st.sidebar.number_input("Poisson's Ratio ν", value=0.3, min_value=0.0, max_value=0.49, step=0.01)
    density = st.sidebar.number_input("Density (kg/m³)", value=7850, min_value=1) * 1e-12
else:
...
if "solved" in st.session_state and st.session_state.solved:
    ...
    with col1:
        ...
        mesh = pv.UnstructuredGrid(cells_pv, [pv.CellType.HEXAHEDRON] * len(cells), points)
    ...
    st.info("👈 Configure parameters in the sidebar and click **Solve** to run the simulation.")
